...
int main() {
    ...
    std::string url = fmt::format("wss://stream.binance.com:9443/ws/{}@bookTicker/{}@bookTicker", 
                                  boost::algorithm::to_lower_copy(target), 
                                  boost::algorithm::to_lower_copy(support));
        ...
        if (msg->type == ix::WebSocketMessageType::Message) {
            try {
                ...
                if (history.count(target) && history.count(support)) {
                    ...
                    // 注文も自動で target を対象にする
                    execute_trade(expectancy, current_price, target, pending_trades);
                }
            } catch (...) {}
        }
    });

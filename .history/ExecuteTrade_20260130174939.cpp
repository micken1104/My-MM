...
void execute_trade(double expectancy, double current_price, std::string symbol, std::vector<TradeData>& pending_trades) {
    ...
    double safety_factor = 0.1; // 「1/10 ケリー」にする
    ...
    if (elapsed % 100 == 0) { // ログが溢れないよう、10回に1回表示
    ...
    if (abs_expectancy > 0.5 && lot_size_usd > 20.0 && elapsed >= 60) { // 最小注文単位以上の時だけ実行(手数料とか，期待値(自信)が低いときに動かないように)
        TradeData order;
        order.symbol = symbol;

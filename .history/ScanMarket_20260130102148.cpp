...
// scan_marketをこのように書き換えると TradeData と連携できます
    ...
    if (res && res->status == 200) {
        ...
        for (auto& symbol_data : data) {
            ...
            if (history.count(symbol)) {
                ...
                // 条件：インバランス急変！
                if (diff > 0.5) {
                    fmt::print(fg(fmt::color::green) | fmt::emphasis::bold, "🔥 [PUMP ALERT] {}\n", symbol);
                    
                    // 1分後の答え合わせリストに追加
                    pending_checks.push_back({
                        symbol,
                        mid_p,             // エントリー価格
                        current_imbalance, // 特徴量1
                        diff,              // 特徴量2
                        0.0,               // Volume (もし別で取得するならここに入れる)
                        now                // 現在時刻
                    });
                }
            }
        ...
        // 1分経ったものがあるかチェックしてCSV保存
        check_pending_trades(current_prices);
    }
}

...
void execute_trade(double expectancy) {
    ...
    if (lot_size_usd > 50.0 && elapsed >= 10) { // 最小注文単位以上の時だけ実行(手数料とか，期待値(自信)が低いときに動かないように)
        // トレード専用リストに追加
        pending_trades.push_back({
            symbol,
            current_price,
            0.0, 0.0, 0.0, // ここはimbalance等。必要ならセット
            std::chrono::steady_clock::now(),
            lot_size_usd, // TradeData構造体に lot_size を追加してください
            (expectancy > 0 ? "LONG" : "SHORT") // side を追加
        });

        fmt::print(fg(fmt::color::yellow), "🚀 [ORDER] {} {} at ${:.2f}\n", 
                   (expectancy > 0 ? "LONG" : "SHORT"), symbol, current_price);

...
void execute_trade(double expectancy) {
    ...
    if (lot_size_usd > 50.0 && elapsed >= 10) { // 最小注文単位以上の時だけ実行(手数料とか，期待値(自信)が低いときに動かないように)
        ...
        trade.lot_size = lot_size_usd;
        trade.side = (expectancy > 0) ? "LONG" : "SHORT";
        trade.entry_time = std::chrono::steady_clock::now();

        pending_checks.push_back(trade);

        fmt::print(fg(fmt::color::yellow), "🚀 [ORDER] {} {} | Price: ${:.2f} | Size: ${:.2f}\n", trade.side, symbol, current_price, lot_size_usd);

...
int main() {
    ...
    // 🔄 ホット・リロード用スレッドを開始
    std::thread updater([&som]() { // 参照で渡す
        while (true) {
            std::this_thread::sleep_for(std::chrono::minutes(30));
            fmt::print(fg(fmt::color::yellow), "🔄 [SYSTEM] Starting periodic re-training...\n");
            
            // Python実行 (パスは環境に合わせて調整)
            int ret = std::system("python train_som.py"); 
            if (ret == 0) {
                // reloadModelメソッドをSOMEvaluatorに実装しておく必要あり
                if (som.loadModel("som_map_weights.csv", "som_expectancy.csv", "som_scaling_params.csv")) {
                    fmt::print(fg(fmt::color::green), "✅ [MODEL] Model Hot-Reloaded successfully.\n");
                }
            } else {
                fmt::print(fg(fmt::color::red), "❌ [MODEL] Training failed.\n");
            }
        }
    });

...
# images_data (400行6列) をCSV保存
np.savetxt("som_map_weights.csv", np.array(images_data), delimiter=",")

# ついでに「どの座標が買いか」のフラグも書き出す
signal_map = np.zeros((width, high))
for (x, y), sig in signals.items():
    if sig == "BUY": signal_map[x, y] = 1
    if sig == "SELL": signal_map[x, y] = -1
np.savetxt("som_signals.csv", signal_map, delimiter=",")
